...
<div class="form-group {{ $errors->has('persona_id') ? 'has-error' : '' }}">
    <label for="persona_id" class="col-md-6 control-label">Persona</label>
    <div class="col-md-12">
        <select class="form-control" id="persona_id" name="persona_id" required>
        	    <option value="" style="display: none;" {{ old('persona_id', optional($paciente)->persona_id ?: '') == '' ? 'selected' : '' }} disabled selected>Select persona</option>
        	@foreach ($personas as $key => $persona)
			    <option value="{{ $key }}" {{ old('persona_id', optional($paciente)->persona_id) == $key ? 'selected' : '' }}>
			    	{{ $persona }}
			    </option>
			@endforeach
        </select>
...
<div class="form-group {{ $errors->has('obrasocial_id') ? 'has-error' : '' }}">
    <label for="obrasocial_id" class="col-md-6 control-label">Obrasocial</label>
    <div class="col-md-12">
...
<div class="form-group {{ $errors->has('antecedentes_familiares') ? 'has-error' : '' }}">
    <label for="antecedentes_familiares" class="col-md-6 control-label">Antecedentes Familiares</label>
    <div class="col-md-12">
...
<div class="form-group {{ $errors->has('antecedentes_patologico') ? 'has-error' : '' }}">
    <label for="antecedentes_patologico" class="col-md-6 control-label">Antecedentes Patologico</label>
    <div class="col-md-12">
...
<div class="form-group {{ $errors->has('antecedentes_nopatologico') ? 'has-error' : '' }}">
    <label for="antecedentes_nopatologico" class="col-md-6 control-label">Antecedentes Nopatologico</label>
    <div class="col-md-12">
